...
## GRASS Location
We will be using the North Carolina state plane location for the majority of today's lab. Login to the server, start GRASS and choose the `nc_spm_08_grass7` LOCATION, and either the 'user1' or start your own MAPSET. As before, we will use the terminal to run GRASS functions on the command line.
...
Now that the MASK is present, all operations will only take into account the non-MASKed cells:
